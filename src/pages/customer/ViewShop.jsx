// src/pages/customer/ViewShop.jsx
...
const ViewShop = () => {
  ...
  const { shopDetails, shopProducts, cart, loading, error } = useSelector(
  ...
  useEffect(() => {
    ...
    return () => dispatch(resetShopDetails());
  }, [dispatch, shopId, user?.uid]);

  // Filter & sort
  useEffect(() => {
    if (!shopProducts?.length) return setFiltered([]);

    let res = [...shopProducts];
